    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={Object.keys(optionsByExpiry).map(expiry => ({
                      expiry: expiry.slice(5), // Remove year for clarity
                  ...
                  >
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="calls" name="Call Options" stackId="a" fill="#8884d8" />
                    <Bar dataKey="puts" name="Put Options" stackId="a" fill="#82ca9d" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
      ...
      {displayMode === 'comprehensive' && (
        <div className="mt-8 bg-blue-50 p-6 rounded-lg">
          <h3 className="text-xl font-semibold mb-4">Trading Strategy Recommendations</h3>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h4 className="text-lg font-semibold mb-2">Stock Directional Analysis</h4>
              {
                stockPrediction.next_day_predictions.linear_regression > stockPrediction.current_price && 
                stockPrediction.next_day_predictions.ransac > stockPrediction.current_price ? (
                  <div className="p-3 bg-green-100 rounded-md">
                    <p className="font-medium text-green-700">Bullish Trend Detected</p>
                    <p className="text-sm mt-1">Both models predict an upward movement in price.</p>
                  </div>
                ) : stockPrediction.next_day_predictions.linear_regression < stockPrediction.current_price && 
                   stockPrediction.next_day_predictions.ransac < stockPrediction.current_price ? (
                  <div className="p-3 bg-red-100 rounded-md">
                    <p className="font-medium text-red-700">Bearish Trend Detected</p>
                    <p className="text-sm mt-1">Both models predict a downward movement in price.</p>
                  </div>
                ) : (
                  <div className="p-3 bg-yellow-100 rounded-md">
                    <p className="font-medium text-yellow-700">Mixed Signals</p>
                    <p className="text-sm mt-1">Models show conflicting predictions.</p>
                  </div>
                )
              }
              ...
              <div className="mt-4">
                <p className="text-sm">
                  ...
                  {
                    stockPrediction.model_performance.linear_regression_rmse < stockPrediction.current_price * 0.03 &&
                    stockPrediction.model_performance.ransac_rmse < stockPrediction.current_price * 0.03 ?
                    'High' : stockPrediction.model_performance.linear_regression_rmse < stockPrediction.current_price * 0.05 &&
                    stockPrediction.model_performance.ransac_rmse < stockPrediction.current_price * 0.05 ?
                    'Medium' : 'Low'
                  }
                </p>
              </div>
            ...
            <div>
              <h4 className="text-lg font-semibold mb-2">Recommended Options Strategy</h4>
              {
                stockPrediction.next_day_predictions.ransac > stockPrediction.current_price ? (
                  <div className="p-3 bg-green-100 rounded-md">
                    <p className="font-medium text-green-700">Bullish Strategy</p>
                    <p className="text-sm mt-1">Consider call options or call spreads for potential upside.</p>
                    {topOptions.filter(o => o.type === 'call').length > 0 && (
                      <p className="text-sm mt-1">
                        Suggested Contract: {topOptions.filter(o => o.type === 'call')[0]?.contract}
                      </p>
                    )}
                  </div>
                ) : (
                  <div className="p-3 bg-red-100 rounded-md">
                    <p className="font-medium text-red-700">Bearish Strategy</p>
                    <p className="text-sm mt-1">Consider put options or put spreads for downside protection.</p>
                    {topOptions.filter(o => o.type === 'put').length > 0 && (
                      <p className="text-sm mt-1">
                        Suggested Contract: {topOptions.filter(o => o.type === 'put')[0]?.contract}
                      </p>
                    )}
                  </div>
                )
              ...
              <div className="mt-4">
                <p className="text-sm">
                  ...
                  {
                    Math.abs(stockPrediction.next_day_predictions.ransac - stockPrediction.current_price) / 
                    stockPrediction.current_price > 0.03 ? 'High' : 
                    Math.abs(stockPrediction.next_day_predictions.ransac - stockPrediction.current_price) / 
                    stockPrediction.current_price > 0.01 ? 'Medium' : 'Low'
                  }
                </p>
              </div>
            </div>
          ...
          <div className="mt-6">
            ...
            <ul className="list-disc pl-5 space-y-2">
              <li>
                Average price prediction suggests a 
                {
                  ((stockPrediction.next_day_predictions.ransac + stockPrediction.next_day_predictions.linear_regression) / 2 - 
                  stockPrediction.current_price) / stockPrediction.current_price > 0 ? ' positive ' : ' negative '
                }
                movement of 
                {
                  Math.abs(((stockPrediction.next_day_predictions.ransac + stockPrediction.next_day_predictions.linear_regression) / 2 - 
                  stockPrediction.current_price) / stockPrediction.current_price * 100).toFixed(2)
                }%.
              </li>
              <li>
                {
                  topOptions.length > 0 ?
                  `The most favorable option contract is ${topOptions[0].contract} with a potential profit of $${topOptions[0].profit_potential.toFixed(2)}.` :
                  'No highly favorable options identified at this time.'
                }
              </li>
              <li>
                Historical volatility suggests 
                {
                  stockPrediction.model_performance.linear_regression_rmse > stockPrediction.current_price * 0.03 ?
                  ' high ' : ' moderate '
                }
      ...
      <div className="mt-8 text-sm text-gray-500">
        <p><strong>Note:</strong> All predictions are based on historical data analysis and mathematical models. 
        Past performance is not indicative of future results. Always conduct your own research before making investment decisions.</p>
      </div>
    </div>
  );
}
